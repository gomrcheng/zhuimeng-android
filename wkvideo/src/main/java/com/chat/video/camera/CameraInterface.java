    ...
    public void doStartPreview(SurfaceHolder holder, float screenProp) {
        ...
        if (mCamera != null) {
            try {
                ...
                // 设置预览格式为YuvImage支持的格式
                if (mParams.getSupportedPreviewFormats().contains(ImageFormat.NV21)) {
                    mParams.setPreviewFormat(ImageFormat.NV21);
                } else if (mParams.getSupportedPreviewFormats().contains(ImageFormat.YUY2)) {
...
//
    ...
    public void startRecord(Surface surface, float screenProp, ErrorCallback callback) {
        ...
        if (previewFormat != ImageFormat.NV21 && previewFormat != ImageFormat.YUY2) {
            // 如果当前格式不被支持，强制使用NV21格式
