...
import com.chat.uikit.signal.SignalChooseActivity;
...
import com.xinbida.wukongim.message.timer.TimerTasks;
...
public class ChatFragment extends WKBaseFragment<FragChatConversationLayoutBinding> {
    ...
    private Disposable timeSyncDisposable;

    private Long serverTimeMillisStar, serverTimeMillis;

    /**
     * 启动时间同步定时器，每30秒请求一次服务器时间
     */
    private void startTimerPing() {
        Observable.interval(0, 15, TimeUnit.SECONDS).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new Observer<>() {
            @Override
    ...
    private void fetchServerTime() {
        ...
        // 这里假设你使用的是你项目中的网络库
        try {
            serverTimeMillisStar = System.currentTimeMillis();
            // 示例：使用一个提供时间戳的公共API
            String timeUrl = WKApiConfig.baseUrl;
        ...
        client.newCall(request).enqueue(new okhttp3.Callback() {
            ...
            public void onFailure(okhttp3.Call call, IOException e) {
                requireActivity().runOnUiThread(() -> {
                    System.out.println("===========更新线路链接耗时");
                    wkVBinding.tvSignalNum.setText("-");
                    wkVBinding.ivSignalIcon.setImageResource(R.mipmap.dian_red);
                });
                android.util.Log.e("ChatFragment", "时间请求失败: " + e.getMessage());
            }

            @Override
            public void onResponse(okhttp3.Call call, okhttp3.Response response) throws IOException {
                serverTimeMillis = System.currentTimeMillis()-serverTimeMillisStar;
                response.close();
                requireActivity().runOnUiThread(() -> {
                    System.out.println("===========更新线路链接耗时:"+serverTimeMillis);
                    wkVBinding.tvSignalNum.setText(""+serverTimeMillis);
                    if(serverTimeMillis<300){
                        wkVBinding.ivSignalIcon.setImageResource(R.mipmap.dian_green);
                    }else if(serverTimeMillis>300 && serverTimeMillis<600){
                        wkVBinding.ivSignalIcon.setImageResource(R.mipmap.dian_yellow);
                    } else{
                        wkVBinding.ivSignalIcon.setImageResource(R.mipmap.dian_red);
                    }
                });
            ...
            android.util.Log.d("ChatFragment", "发送时间同步请求: " + timeUrl);
        } catch (Exception e) {
    ...
    private void stopTimeSyncTimer() {
        if (timeSyncDisposable != null) {
            timeSyncDisposable.dispose();
            timeSyncDisposable = null;
        }
    }
...
//        startTimeSyncTimer();
        ...
        wkVBinding.llSignal.setOnClickListener(v -> {
            startActivity(new Intent(getContext(),SignalChooseActivity.class));
        });
    }
