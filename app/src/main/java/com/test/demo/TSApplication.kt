...
import com.chat.wkredpacket.WKRedpacketApplication
import com.chat.wktransfer.WKTransferApplication
import com.chat.wkwallet.WKWalletApplication
...
class TSApplication : MultiDexApplication() {
    ...
    private fun initAll() {
        CrashReport.initCrashReport(this, "1d55d6d3da", true) //腾讯bugly
...
//        WKPushApplication.getInstance().init(getAppPackageName(), this)//推送暂时不需要
        WKRichApplication.instance.init();//富文本模块
        WKPinnedMessageApplication.init();//消息置顶模块
        WKAdvancedApplication.instance.init();//旗舰模块（消息已读未读，消息点赞，消息编辑，截屏消息，聊天搜索，消息回执，聊天背景）
        WKImageEditorApplication.getInstance().init()//图片编辑模块
        WKGroupManageApplication.getInstance().init();//群管模块
        WKFavoriteApplication.instance.init();//收藏模块
        WKLabelApplication.instance.init(this);//标签模块
        WKVideoApplication.getInstance().init(this);//小视频模块
        WKStickerApplication.instance.init()//表情
        ...
        WKCommonModel.getInstance().getAppConfig { _, _, appConfig ->
            if (appConfig != null) {
                var webRtcJson = appConfig.webrtc as JSONObject;
                var agoraJson = appConfig.agora as JSONObject;
                val iceServer = PeerConnection.IceServer.builder(
                    // rtc服务器地址
                    webRtcJson.getString("turn_server")
                    // 用户名
                ).setUsername(webRtcJson.getString("turn_username")).setPassword(
                    webRtcJson.getString("turn_password") //密码
                ).createIceServer()
                val iceServers: ArrayList<PeerConnection.IceServer> = ArrayList()
                iceServers.add(iceServer)
                WKUIRTCApplication.init(iceServers);// 音视频
            }
        }
    }
    ...
    //自动切换节点
    private fun initApi() {
        WKSharedPreferencesUtil.getInstance().putSP("api_config_json",WKApiConfig.lineConfig)
        var apiURL = WKSharedPreferencesUtil.getInstance().getSP("api_base_url","")
        if (TextUtils.isEmpty(apiURL)) {
//            apiURL = "http://206.238.115.37:8090"//模拟主线路不通
//            apiURL = "http://206.238.115.47:8090"
            apiURL = WKApiConfig.defaultUrl;
    ...
    private fun getList(): ArrayList<PeerConnection.IceServer> {
        val iceServer = PeerConnection.IceServer.builder(
            // rtc服务器地址
            "turn:47.238.150.73:3478?transport=udp"
            // 用户名
        ).setUsername("solchat").setPassword(
            "solchatpasswd" //密码
        ).createIceServer()
        val iceServers: ArrayList<PeerConnection.IceServer> = ArrayList()
        iceServers.add(iceServer)
