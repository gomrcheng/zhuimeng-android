...
//        wkVBinding.webView.loadUrl("file:///android_asset/web/report.html");
        if (url.contains(WKApiConfig.baseWebUrl + "report.html")) {
...
//            int startIndex = url.indexOf(qrcodePrefix);
//            if (startIndex != -1) {
//                // 提取包括 qrcode/ 的部分
//                url = "http://api1.koi.ac/" + url.substring(startIndex);
//            }
//        }
        Log.e("加载的URL", url);
        Map<String, String> headers = new HashMap<>();
        headers.put("token", WKConfig.getInstance().getToken());
        wkVBinding.webView.loadUrl(url,headers);
    }
    ...
    protected void initListener() {
        ...
        wkVBinding.webView.setWebChromeClient(new WebChromeClient() {
            ...
            public void onReceivedTitle(WebView webView, String s) {
                ...
                if (!TextUtils.isEmpty(s) && !"about:blank".equals(s) && !gone) {
                    titleTv.setText(s);
                }
            }
